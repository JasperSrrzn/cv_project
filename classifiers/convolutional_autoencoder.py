...
def conv2d_block(input_tensor, n_filters, kernel_size=3, batchnorm=True):
    x =Conv2D(filters=n_filters, kernel_size=(kernel_size,kernel_size),kernel_initializer='he_normal',padding='same',kernel_regularizer = regularizers.l1_l2(l1=0.01, l2=0.01))(input_tensor)
    if batchnorm:
        x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Conv2D(filters=n_filters, kernel_size=(kernel_size,kernel_size),kernel_initializer='he_normal',padding='same',kernel_regularizer = regularizers.l1_l2(l1=0.01, l2=0.01))(x)
    if batchnorm:
        x = BatchNormalization()(x)
    x = Activation('relu')(x)
    return x
...
class ConvolutionalAutoencoder(object):
    def __init__(self,latent_dim,n_filters,pretrained_weights=None):
        ...
        if pretrained_weights==None:
            ...
            f2 = Dense(int(shape[1])*int(shape[2])*int(shape[3]),kernel_regularizer = regularizers.l1_l2(l1=0.01, l2=0.01))(latent)
            ...
            outputs = Conv2D(3 , (1,1), activation='sigmoid',kernel_regularizer = regularizers.l1_l2(l1=0.01, l2=0.01))(conv9)
